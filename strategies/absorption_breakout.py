...
try:
    from MQScore_6D_Engine_v3 import MQScoreEngine, MQScoreComponents, MQScoreConfig
...
class AbsorptionBreakoutNexusAdapter:
    ...
    def execute(
        self, market_data: Any, features: Optional[Dict] = None
    ) -> Dict[str, Any]:
        """
            ...
            try:
                ...
                if self.mqscore_engine:
                    try:
                        ...
                        price = float(
                            market_data.get("close", market_data.get("price", 0))
                        )
                        ...
                        mqscore_result = self.mqscore_engine.calculate_mqscore(
                            market_df
                        )
                        ...
                        if mqscore_quality < self.mqscore_threshold:
                            logger.info(
                                f"MQScore REJECTED: quality={mqscore_quality:.3f} < {self.mqscore_threshold}"
                            )
                            return {
                                "signal": 0.0,
                                "confidence": 0.0,
                        ...
                        logger.warning(
                            f"MQScore calculation error: {e} - proceeding without MQScore filter"
                        )
                        mqscore_quality = None
                ...
                features.update(
                    {
                        "absorption_level": getattr(
                            signal_result, "validation_score", 0.0
                        ),
                        "false_breakout_score": getattr(
                            signal_result, "false_breakout_score", 0.0
                        ),
                        "regime": getattr(
                            signal_result, "market_regime", MarketRegime.UNKNOWN
                        ).value,
