...
def check_nexus_ai_availability():
    ...
    try:
        # Avoid circular import by checking if we're being imported by nexus_ai
        import sys
        if 'nexus_ai' in sys.modules:
            # We're being imported by nexus_ai, so it's available
            NEXUS_AI_AVAILABLE = True
            logger = logging.getLogger(__name__)
            logger.info("✓ NEXUS AI components successfully detected")
            return True
        else:
            # Try to import nexus_ai directly (for standalone usage)
            import nexus_ai
